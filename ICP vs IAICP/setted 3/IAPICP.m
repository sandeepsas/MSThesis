function [TR, TT, ER] = IAPICP(q,p,iter,wr)
...
%     end
    ...
    [R,T] = eq_plane(q(1:3,q_idx),pt(1:3,p_idx),Normals(:,q_idx),weights(p_idx));
...
if str2double(v.Version) >= 7.3 
    neighbors = transpose(knnsearch(transpose(pman), transpose(pman), 'k', k+1));
else
    neighbors = kNearestNeighbors(pman, pman, k+1);
end

for i = 1:m
    x = pman(:,neighbors(2:end, i));
    p_bar = 1/k * sum(x,2);
